...
function arm(flags) {
  ...
  const request = validateRequest({
    condition,
    memo: flags.memo,
    ...(integerFlag(flags, 'interval-seconds') === undefined
      ? {}
      : { interval_seconds: integerFlag(flags, 'interval-seconds') }),
    ...(integerFlag(flags, 'timeout-seconds') === undefined
      ? {}
      : { timeout_seconds: integerFlag(flags, 'timeout-seconds') }),
    ...(flags['on-timeout'] === undefined
      ? {}
      : { on_timeout: flags['on-timeout'] }),
  });
  ...
  const state = {
    ...
    ...(flags['wake-mode'] === undefined
      ? {}
      : { wakeMode: flags['wake-mode'] }),
  };
  if (
...
function flushHandle(handle) {
  ...
  if (killTmuxSession(state.tmuxSession)) {
    return {
      ...
      reason: 'stale tmux session removed',
    };
  }
  return {
    handle,
...
function flush(flags) {
  if (!flags.handle) throw new Error('--handle is required');
  console.log(
    JSON.stringify({ ok: true, ...flushHandle(flags.handle) }, null, 2),
  );
}
