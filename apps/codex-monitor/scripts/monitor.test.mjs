...
test(
  'tmux boomerang environment self-tears down after wake completion',
  { timeout: 10_000 },
  async (context) => {
    ...
    const monitorHome = mkdtempSync(
      join(tmpdir(), 'codex-monitor-integration-'),
    );
    ...
    assert.equal(
      armedResult.status,
      0,
      armedResult.stderr || armedResult.stdout,
    );
...
test(
  'production wake waits for host dispatch and can be polled and acknowledged',
  { timeout: 10_000 },
  async (context) => {
    if (spawnSync('tmux', ['-V']).status !== 0) {
      context.skip('tmux is not installed');
    ...
    const monitorHome = mkdtempSync(
      join(tmpdir(), 'codex-monitor-host-dispatch-'),
    );
    const monitor = fileURLToPath(new URL('./monitor', import.meta.url));
    ...
    try {
      for (let attempt = 0; attempt < 50; attempt += 1) {
        ...
        if (
          state.state === 'met' &&
          state.wake.status === 'awaiting_host_dispatch'
        )
          break;
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
      ...
      assert.equal(
        pollResult.status,
        0,
        pollResult.stderr || pollResult.stdout,
      );
      ...
      const rejectedAcknowledge = spawnSync(
        monitor,
      ...
      assert.match(
        rejectedAcknowledge.stderr,
        /--delivery host-message-accepted is required/,
      );

      const acknowledgeResult = spawnSync(
        monitor,
        [
          'acknowledge',
          '--handle',
      ...
      assert.equal(
        acknowledgeResult.status,
        0,
        acknowledgeResult.stderr || acknowledgeResult.stdout,
      );
