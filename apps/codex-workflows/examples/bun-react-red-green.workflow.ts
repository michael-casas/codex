...
Do not emit commentary, progress updates, or interim assistant messages. Perform the work first, then emit exactly one final schema-bound JSON response.
Remediate only the FAIL findings in the supplied independent audit at the exact project path. Never invoke npm, npx, pnpm, or Yarn. Do not alter package.json, bun.lock, public files, configuration, or dependencies and do not create foreign lockfiles. Edit only src/App.jsx and src/App.test.jsx. Preserve the exact Workflow Proof h1. Add one element with data-testid="audit-remediation-status" and exact visible text Audit findings resolved, then add a bun:test assertion for it. Set BUN_INSTALL_CACHE_DIR to ${JSON.stringify(resolve(proofRoot, 'bun-cache'))}, then run bun test and bun run build to terminal completion. Return only schema-bound JSON with status READY_FOR_EXTERNAL_AUDIT, the exact projectPath, addressedFindings exactly ["BUN-REACT-AUDIT-003", "BUN-REACT-AUDIT-004"], and actual terminal exit codes.`,
