...
Remediate only the FAIL findings in the supplied independent audit at the exact supplied project path. Do not alter package.json, package-lock.json, public files, configuration, or dependencies. Edit only src/App.js and src/App.test.js. Preserve the exact Workflow Proof h1. Add one element with data-testid="audit-remediation-status" and exact visible text Audit findings resolved, then add a native React Testing Library assertion for it. Run CI=true npm test -- --watchAll=false and npm run build. Return only schema-bound JSON with the exact status READY_FOR_EXTERNAL_AUDIT, the exact projectPath, the addressed failing IDs, and command exit codes.`,
