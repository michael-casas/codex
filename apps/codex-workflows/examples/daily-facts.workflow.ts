...
import {
  dailyFactsDisallowedHosts,
  dailyFactsPreferredSourceHosts,
  dailyFactsSourceClusters,
  renderDailyFacts,
  validateDailyFacts,
...
For slot ${assignment.slot}, search these compatible publisher hosts first: ${dailyFactsSourceClusters[assignment.slot].join(', ')}. If one candidate has not yielded four qualifying direct pages within four focused searches, move to the next candidate. Once four fully verified sources support one coherent topic, stop searching and return immediately.
