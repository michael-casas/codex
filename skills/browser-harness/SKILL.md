...
- `BH_PATH=~/browser-harness/`
- **all paths below are relative to BH_PATH above**
...
**Founder-mandated sequence for ALL visual verification:**
1. `browser_navigate(url)`
2. `browser_console(clear=True)` — clear initial noise
3. `browser_refresh()` — first refresh (handles deferred useContext bug)
4. `browser_console(clear=True)` — clear stabilized state
5. `browser_refresh()` — second refresh (real errors persist)
6. `browser_console()` — MUST be empty; persistent errors block gates
7. `browser_vision("describe layout, verify no visual errors")`

See `references/verification-protocol.md` for full details. This replaces any
self-certified visual claims — this protocol is the only acceptable evidence.
...
## Discord Agent-Turn Injection

See `references/discord-agent-injection.md` — use the browser harness to post
user messages to Discord web, triggering a new agent turn in the same thread.
This is the cleanest autonomous pipeline feedback mechanism: agent finishes work
→ cmux notification fires → browser harness navigates Discord web → types a
message as the user → agent wakes up in the same thread with full tool access.

Requires the browser-harness Chrome profile to be logged into Discord (user
