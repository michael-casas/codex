#!/usr/bin/env python3
"""Verify and close the canonical SQLite compaction checkpoint."""
...
def main() -> int:
    try:
        event = parse_hook_event("PostCompact", sys.stdin.read())
        ...
        conn = initialize(db_path, session_id, must_exist=True)
        try:
            record_postcompact(conn, session_id=session_id, turn_id=turn_id, trigger=trigger)
        finally:
            conn.close()
        return 0
