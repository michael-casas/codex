...
export function runCommand(command, signal) {
  return new Promise((resolveCommand, rejectCommand) => {
    ...
    const abort = () => {
      ...
      setTimeout(() => {
        if (!settled && child.exitCode === null) signalOwnedProcess(child, "SIGKILL");
      }, 150).unref();
    };
    signal.addEventListener("abort", abort, { once: true });
...
export async function waitForCondition(condition, intervalMilliseconds, signal, onObservation) {
  switch (condition.kind) {
    ...
    case "file_exists": {
      ...
      return poll(
        () => Promise.resolve(existsSync(path) ? { met: true, details: { path } } : { met: false }),
    ...
    case "file_matches": {
      const path = resolve(condition.path);
      const pattern = condition.pattern === undefined ? undefined : new RegExp(condition.pattern);
      return poll(async () => {
        ...
        if (pattern) {
          ...
          if (match) return { met: true, details: { path, matched: match[0] } };
        }
        if (condition.contains !== undefined && content.includes(condition.contains)) {
          return { met: true, details: { path, matched: condition.contains } };
        }
        return { met: false };
      }, intervalMilliseconds, signal, onObservation);
    }
    case "process_exit":
      return poll(
        () => Promise.resolve(probeProcess(condition.pid)
          ? { met: false }
