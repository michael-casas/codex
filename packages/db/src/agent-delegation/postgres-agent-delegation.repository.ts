...
type Row = { delegation_id: string; execution_id: string; agent_id: string; request_sha256: string; command: DelegateAgentCommand; state: DelegationState; workspace_ref: string | null; thread_id: string | null; session_id: string | null; active_turn_id: string | null; replayed?: boolean };
const record = (row: Row): DelegationRecord => ({ delegationId: row.delegation_id, executionId: row.execution_id, agentId: row.agent_id, fingerprint: row.request_sha256, command: row.command, state: row.state, ...(row.workspace_ref ? { workspaceRef: row.workspace_ref } : {}), ...(row.thread_id ? { threadId: row.thread_id } : {}), ...(row.session_id ? { sessionId: row.session_id } : {}), ...(row.active_turn_id ? { activeTurnId: row.active_turn_id } : {}) });
